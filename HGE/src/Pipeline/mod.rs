...
impl EnginePipelines
{
	...
	{
		return SINGLETON.get_or_init(|| EnginePipelines::new());
	}

	pub fn pipelineCreation(
		&self,
		name: impl Into<String>,
		transparency: bool,
		renderpass: Arc<RenderPass>,
		subpassID: u32,
		vertexDef: impl VertexDefinition,
	) -> Arc<GraphicsPipeline>
	{
		...
		let mut specialzitiondata = HashMap::new();
		specialzitiondata.insert(
			0,
			match transparency
			{
		...
		let shadercontent = Shaders::Manager::ManagerShaders::singleton()
			.get(name)
			.unwrap();
		let vertexbinding = shadercontent
			.shader
			.get(&Shader_type::VERTEX)
			.unwrap()
			.entry_point("main")
			.unwrap();
		let fragbinding = shadercontent
			.shader
			.get(&Shader_type::FRAGMENT)
			.unwrap()
			.specialize(specialzitiondata)
			.unwrap()
			.entry_point("main")
			.unwrap();
		let vertexinputstate = vertexDef.definition(&vertexbinding).unwrap();
