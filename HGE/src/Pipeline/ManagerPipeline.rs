...
impl ManagerPipeline
{
	...
	pub fn addFunc(&self, name: impl Into<String>, pipeline: impl Fn(Arc<RenderPass>, bool) -> Arc<GraphicsPipeline> + Send + Sync + 'static,
	               topology: PrimitiveTopology,
	               haveTransparency: bool)
	{
		self._updateFunc.insert(name.into(),updateStruct{
	...
	{
		return self.get(name.into()).map(|x|{
			x.pipeline.layout().clone()
		});
	}
	...
	pub fn layoutGetDescriptor(&self, name: impl Into<String>, index: usize) -> Option<Arc<DescriptorSetLayout>>
	{
		...
		{
			return pipelineDatas.pipeline.layout().set_layouts().get(index).map(|x|{
				x.clone()
			});
		}
	...
	pub fn pipelineRefresh(&self, renderpass: Arc<RenderPass>)
	{
		...
		{
			self._pipelines.insert(x.key().clone(),PipelineDatas{
			...
			if(x.transparency)
			{
				self._pipelinesTransparency.insert(x.key().clone(),PipelineDatas{
					primitive_type: x.topology,
