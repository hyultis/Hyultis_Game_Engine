...
impl HGEconfig
{
	...
	pub fn singleton() -> &'static Self
	{
		...
		{
			Some(val) => val,
			_ => panic!("need to \"defineGeneral\" before anything else"),
		}
	}
	...
	{
		let mut config =
			HConfigManager::singleton().get(self._generalConfig.load().configName.clone());
		let mut updatedconfig = *self._system_video.load_full().clone();

		updatedconfig.setPresentModeString(
			config
				.getOrSetDefault(
					"system/swapchain/presentmode",
					JsonValue::String(
						match defaultPresentmode
						{
		...
		updatedconfig.fpslimiter = config
			.getOrSetDefault(
				"system/swapchain/fpslimiter",
				JsonValue::from(defaultFpslimiter),
			)
			.as_u64()
			.unwrap_or(0) as u8;
	...
	pub fn saveSwapchainFromConfig(&self)
	{
		...
		config.set(
			"system/swapchain/presentmode",
			updatedconfig.getPresentModeString(),
		);
