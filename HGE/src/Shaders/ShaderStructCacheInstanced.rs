	...
	I: BufferContents,
{
	...
	{
		ShaderStructHolder_utils::updateBuffer(
			vertex,
			&mut self._cacheDatasMem,
			BufferCreateInfo {
		...
		self._cacheIndicesLen = ShaderStructHolder_utils::updateBuffer(
			indices,
			&mut self._cacheIndicesMem,
			BufferCreateInfo {
		...
		self._cacheInstanceLen = ShaderStructHolder_utils::updateBuffer(
			intance,
			&mut self._cacheInstanceMem,
			BufferCreateInfo {
				usage: BufferUsage::VERTEX_BUFFER,
				..Default::default()
			},
	...
	{
		if (self._cacheIndicesLen == 0 || self._cacheInstanceLen == 0)
		{
			return;
		}
		let Some(datamem) = &self._cacheDatasMem
		else
		{
			return;
		};
		let Some(indicemem) = &self._cacheIndicesMem
		else
		{
			return;
		};
		let Some(instancemem) = &self._cacheInstanceMem
		else
		{
			return;
		};
		...
		unsafe {
			cmdBuilder
				.bind_vertex_buffers(0, (datamem.clone(), instancemem.clone()))
				.unwrap()
				.bind_index_buffer(indicemem.clone())
				.unwrap()
				.draw_indexed(self._cacheIndicesLen, self._cacheInstanceLen, 0, 0, 0)
				.unwrap();
		}
