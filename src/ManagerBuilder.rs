...
use vulkano::command_buffer::{AutoCommandBufferBuilder, CommandBufferInheritanceInfo, CommandBufferUsage, RenderPassBeginInfo, SecondaryAutoCommandBuffer, SubpassBeginInfo};
...
impl ManagerBuilder
{
	...
	pub fn add(&self, name: &str, commandBuffer: Arc<StandardCommandBufferAllocator>, queue: Arc<Queue>, usage: CommandBufferUsage)
	{
		self._builders.write().insert(name.to_string(), ManagerBuilderStorage {
	...
	pub fn generate(&self, name: &str) -> Option<AutoCommandBufferBuilder<SecondaryAutoCommandBuffer<Arc<StandardCommandBufferAllocator>>, Arc<StandardCommandBufferAllocator>>>
	{
	...
	pub fn builderBegin(&self,builder: &mut AutoCommandBufferBuilder<SecondaryAutoCommandBuffer, Arc<StandardCommandBufferAllocator>>,framebuffers: Arc<Framebuffer>) -> anyhow::Result<()>
	{
	...
	pub fn builderAddPipeline(builder: &mut AutoCommandBufferBuilder<SecondaryAutoCommandBuffer<Arc<StandardCommandBufferAllocator>>, Arc<StandardCommandBufferAllocator>>, pipelineName: impl Into<String>)
	{
		...
		{
			None => {
				HTrace!("pipeline \"{}\" doesn't exist in ManagerPipeline",pipelineName);
			}
			Some(pipelineDatas) => {
